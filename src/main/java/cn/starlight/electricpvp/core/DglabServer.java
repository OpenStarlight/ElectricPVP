...
import cn.starlight.electricpvp.core.util.ClientInfo;
...
import static cn.starlight.electricpvp.core.DglabClient.waveformMap;

public class DglabServer extends WebSocketServer {
    ...
    public DglabServer(InetSocketAddress address) {
        super(address);
    }
    ...
    public void onMessage(WebSocket conn, String message) {
        ...
        if (tmp.getMessage().equals("DGLAB") && tmp.getType().equals("bind") && tmp.getClientId().equals("1234-123456789-12345-12345-01") && tmp.getTargetId().equals(clientInfo.getClientId())) {
            ...
            if (Number != 405) dgStrength.setBMaxStrength(Number);
        }
    }
    ...
    public void onError(WebSocket conn, Exception ex) {
        if (ex instanceof BindException) {
            ElectricPVP.showMsg("§cAddress/Port already in use! Failed to start server!");
